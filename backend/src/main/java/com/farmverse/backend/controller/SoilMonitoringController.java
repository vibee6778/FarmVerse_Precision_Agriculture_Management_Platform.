...
public class SoilMonitoringController {
    ...
    public ResponseEntity<?> getCropRecommendations(@PathVariable Long farmId) {
        ...
        // Evaluate NPK Deficits and calculate exact stoichiometric dosage per acre
        double targetN = 50.0;
        double targetP = 25.0;
        double targetK = 40.0;
        double acres = farm.getSizeAcres() != null ? farm.getSizeAcres() : 1.0;

        double deficitN = Math.max(0, targetN - avgN);
        double deficitP = Math.max(0, targetP - avgP);
        double deficitK = Math.max(0, targetK - avgK);

        // Urea (46% N), Single Super Phosphate (16% P), Muriate of Potash (60% K)
        double ureaKg = Math.round((deficitN / 0.46) * acres * 10.0) / 10.0;
        double sspKg = Math.round((deficitP / 0.16) * acres * 10.0) / 10.0;
        double mopKg = Math.round((deficitK / 0.60) * acres * 10.0) / 10.0;

        Map<String, Object> fertilizerDosage = new LinkedHashMap<>();
        fertilizerDosage.put("ureaKg", ureaKg);
        fertilizerDosage.put("sspKg", sspKg);
        fertilizerDosage.put("mopKg", mopKg);
        ...
        if (deficitN > 0) {
            ...
            fertilizerActions.add(String.format("Apply %.1f kg Urea (N: 46%%) for %.1f acres.", ureaKg, acres));
        }
        if (deficitP > 0) {
            deficiencies.add("Phosphorus (P)");
            fertilizerActions.add(String.format("Apply %.1f kg Single Super Phosphate (P: 16%%) for %.1f acres.", sspKg, acres));
        }
        if (deficitK > 0) {
            deficiencies.add("Potassium (K)");
            fertilizerActions.add(String.format("Apply %.1f kg Muriate of Potash (K: 60%%) for %.1f acres.", mopKg, acres));
        }
        ...
        response.put("fertilizerDosageKg", fertilizerDosage);
